...
class Database:
    ...
    async def get_pending_withdrawal_orders(self) -> List[Dict]:
    ...
    async def update_order_status(self, order_id: str, status: str) -> bool:
        result = await self.orders.update_one({'order_id': order_id}, {'$set': {'status': status}})
        return result.modified_count > 0
    ...
    async def create_order(self, user_id: int, card_name: str, amount: float, address: str) -> Dict:
        new_order = {
        ...
        await self.orders.insert_one(new_order)
        return new_order
...
class Database:
    ...
    async def ban_user(self, user_id: int) -> bool:
        result = await self.users.update_one({'user_id': user_id}, {'$set': {'is_banned': True}})
        return result.modified_count > 0

    async def unban_user(self, user_id: int) -> bool:
        result = await self.users.update_one({'user_id': user_id}, {'$set': {'is_banned': False}})
        return result.modified_count > 0
